...
public class Blinky : Sprite
{
    ...
    public override Directions GetMovementDirection()
    {
        ...
        Vector2 nextTile;
        nextTile.x = Mathf.Round( (rb.position.x + movementDir.x * 0.25f) * 2.0f) / 2.0f;
        nextTile.y = Mathf.Round( (rb.position.y + movementDir.y * 0.25f) * 2.0f) / 2.0f;
        ...
        lastDirection = GetDirectionFromVector(bestChoice - nextTile);
        return lastDirection;
    }
